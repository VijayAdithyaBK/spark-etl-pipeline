...
class StreamingSimulator:
    ...
    def __init__(self, output_dir: Path, records_per_batch: int = 100, interval_seconds: float = 1.0):
    ...
    def start(self, num_batches: Optional[int] = None) -> None:
        ...
        self._thread.start()
        logger.info(f"Started streaming simulator: {self.records_per_batch} records every {self.interval}s")
    ...
    def _run(self, num_batches: Optional[int] = None) -> None:
        """Internal run loop."""
        while self._running and (num_batches is None or self._batch_count < num_batches):
            self._write_batch()
...
class MicroBatchProcessor:
    ...
    def __init__(self, input_dir: Path, processed_dir: Optional[Path] = None):
        ...
        self.processed_dir = Path(processed_dir) if processed_dir else input_dir / "_processed"
