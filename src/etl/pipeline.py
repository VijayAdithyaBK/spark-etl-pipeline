...
class ETLPipeline:
    ...
    def _transform(self, df: DataFrame) -> DataFrame:
        ...
        if "amt" in df.columns:
            chain.add_custom(
                lambda d: d.withColumnRenamed("amt", "amount"), "rename_amount"
            )

        # Add amount transformer
        chain.add(
            AmountTransformer(
                "amount" if "amount" in df.columns or "amt" in df.columns else "amt"
            )
        )
    ...
    def _load(self, df: DataFrame) -> None:
        ...
        if self.config.output_format == "parquet":
            ...
            loader.load(
                df, self.config.output_path, partition_by=self.config.partition_by
            )
...
if __name__ == "__main__":
    ...
    config = PipelineConfig(
        name="test_pipeline", input_path="data/raw/transactions.csv"
    )
