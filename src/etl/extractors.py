...
class ParquetExtractor(DataExtractor):
    ...
    def extract(self, path: str | Path, columns: Optional[list[str]] = None, **options) -> DataFrame:
...
class JSONExtractor(DataExtractor):
    ...
    def extract(self, path: str | Path, multiline: bool = False, schema: Optional[StructType] = None, **options) -> DataFrame:
        """Extract data from JSON file(s)."""
        logger.info(f"Extracting JSON from: {path}")
        
        reader = self.spark.read.format("json").option("multiline", str(multiline).lower())
...
class IncrementalExtractor(DataExtractor):
    ...
    def extract(
        ...
        **options
        ...
        logger.info(f"Incremental extraction from {path}, watermark column: {watermark_column}")
