...
class CleansingTransformer(DataTransformer):
    ...
    def __init__(self, drop_duplicates: bool = True, drop_nulls: bool = False, subset: Optional[list[str]] = None):
...
class DateTransformer(DataTransformer):
    ...
    def transform(self, df: DataFrame) -> DataFrame:
        ...
        return df.withColumn("year", F.year(col)) \
                 ...
                 .withColumn("is_weekend", F.when(F.dayofweek(col).isin([1, 7]), True).otherwise(False))
...
class AmountTransformer(DataTransformer):
    ...
    def transform(self, df: DataFrame) -> DataFrame:
        if self.amount_column not in df.columns:
            logger.warning(f"Column {self.amount_column} not found, skipping AmountTransformer")
            return df
...
class FraudFeaturesTransformer(DataTransformer):
    ...
    def transform(self, df: DataFrame) -> DataFrame:
        ...
        if "amount" in df.columns:
            df = df.withColumn("is_high_amount", F.when(F.col("amount") > 1000, True).otherwise(False))
        
        # Late night transaction flag
        if "hour" in df.columns:
            df = df.withColumn("is_late_night", F.when((F.col("hour") >= 23) | (F.col("hour") <= 5), True).otherwise(False))
...
class TransformerChain:
    ...
    def add_custom(self, func: Callable[[DataFrame], DataFrame], name: str = "custom") -> "TransformerChain":
