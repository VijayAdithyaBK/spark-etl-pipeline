...
class Rule:
    ...
    condition: Callable[
        [DataFrame], DataFrame
    ]  # Returns filtered DataFrame of failures
    ...
    def check(self, df: DataFrame) -> ValidationResult:
        ...
        passed = failed == 0
        message = (
            f"Rule '{self.name}': {failed}/{total} records failed"
            if not passed
            else f"Rule '{self.name}': passed"
        )

        logger.log(
            "WARNING" if not passed and self.severity == "warning" else "INFO", message
        )
