...
class FraudDetectionModel:
    ...
    def __init__(
        ...
        self.categorical_features = (
            categorical_features or self.DEFAULT_CATEGORICAL_FEATURES
        )
        self._model = None
        self._train_df = None
        self._test_df = None
    ...
    def prepare_features(self) -> DataFrame:
        ...
        available_categorical = [
            f for f in self.categorical_features if f in df.columns
        ]

        logger.info(
            f"Using {len(available_numeric)} numeric features: {available_numeric}"
        )
        logger.info(
            f"Using {len(available_categorical)} categorical features: {available_categorical}"
        )
    ...
    def train(self, n_trees: int = 100, max_depth: int = 10) -> "FraudDetectionModel":
        ...
        for cat_col in self._available_categorical:
            indexer = StringIndexer(
                inputCol=cat_col, outputCol=f"{cat_col}_indexed", handleInvalid="keep"
            )
    ...
    def evaluate(self) -> ModelMetrics:
        ...
        feature_importance = [
            ...
            for feat, imp in sorted(
                zip(self._feature_cols, importances), key=lambda x: x[1], reverse=True
            )
        ][
            :15
        ]  # Top 15 features
