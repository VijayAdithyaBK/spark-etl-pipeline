...
class WindowAnalytics:
    ...
    def __init__(
        self, df: DataFrame, partition_by: str | list[str], order_by: str | list[str]
    ):
        self._df = df
        ...
        self._window = Window.partitionBy(
            *[F.col(c) for c in self._partition_cols]
        ).orderBy(*[F.col(c) for c in self._order_cols])
        self._unbounded = self._window.rowsBetween(
    ...
    def add_rank(self, alias: str = "rank", dense: bool = False) -> "WindowAnalytics":
        self._df = self._df.withColumn(
            alias, (F.dense_rank() if dense else F.rank()).over(self._window)
        )
        return self
    ...
    def add_lag(
        self, column: str, offset: int = 1, alias: Optional[str] = None
    ) -> "WindowAnalytics":
        out = alias or f"{column}_lag_{offset}"
        self._df = self._df.withColumn(
            out, F.lag(F.col(column), offset).over(self._window)
        )
        return self
    ...
    def add_lead(
        ...
        self._df = self._df.withColumn(
            out, F.lead(F.col(column), offset).over(self._window)
        )
        return self
    ...
    def add_percent_change(
        ...
        self._df = self._df.withColumn(
            out,
            F.when(prev.isNull() | (prev == 0), None).otherwise(
                ((F.col(column) - prev) / prev) * 100
            ),
        )
        return self

    def add_moving_average(
        self, column: str, window_size: int = 7, alias: Optional[str] = None
    ) -> "WindowAnalytics":
...
def lag_analysis(
    ...
    for period in lag_periods:
        df = df.withColumn(
            f"{value_column}_lag_{period}",
            F.lag(F.col(value_column), period).over(window),
        )
    return df
...
def session_analysis(
    ...
    df = df.withColumn(
        "_diff_min",
        (
            F.unix_timestamp(F.col(timestamp_column))
            - F.unix_timestamp(F.col("_prev_ts"))
        )
        / 60,
    )
    df = df.withColumn(
        "_new_session",
        F.when(
            F.col("_prev_ts").isNull() | (F.col("_diff_min") > session_gap_minutes), 1
        ).otherwise(0),
    )
