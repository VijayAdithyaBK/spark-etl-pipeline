...
class PartitionOptimizer:
    ...
    def analyze_partitions(df: DataFrame) -> dict:
        ...
        return {
            ...
            "avg_size": (
                sum(partition_sizes) / len(partition_sizes) if partition_sizes else 0
            ),
        }
    ...
    def repartition_by_size(
        ...
        logger.info(
            f"Repartitioning to {target_partitions} partitions (estimated {estimated_total_mb:.1f}MB)"
        )
        return df.repartition(target_partitions)

    @staticmethod
    def coalesce_if_needed(
        df: DataFrame, min_rows_per_partition: int = 10000
    ) -> DataFrame:
        ...
        if stats["avg_size"] < min_rows_per_partition and stats["num_partitions"] > 1:
            target = max(
                1,
                int(
                    stats["num_partitions"] * stats["avg_size"] / min_rows_per_partition
                ),
            )
            logger.info(
                f"Coalescing from {stats['num_partitions']} to {target} partitions"
            )
...
class CacheManager:
    ...
    def get_cache_info(self) -> dict:
        ...
        return {
            "cached_dataframes": list(self._cached_dfs.keys()),
            "count": len(self._cached_dfs),
        }
...
def optimize_for_join(
    df: DataFrame, join_columns: list[str], num_partitions: Optional[int] = None
) -> DataFrame:
...
def repartition_by_key(
    df: DataFrame, key_column: str, num_partitions: int = 200
) -> DataFrame:
