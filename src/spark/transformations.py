...
def add_processing_metadata(
    ...
    """
    result = (
        df.withColumn("_processing_timestamp", F.lit(datetime.now().isoformat()))
        .withColumn("_pipeline_name", F.lit(pipeline_name))
        .withColumn("_spark_partition_id", F.spark_partition_id())
    )

    if include_row_id:
        result = result.withColumn("_row_id", F.monotonically_increasing_id())
...
def standardize_dates(
    ...
    for col_name in date_columns:
        ...
        # Parse to timestamp
        df = df.withColumn(col_name, F.to_timestamp(F.col(col_name), input_format))

        # Optionally reformat to string
        if output_format:
            df = df.withColumn(col_name, F.date_format(F.col(col_name), output_format))
...
class DataFrameTransformations:
    ...
    def add_derived_columns(self) -> "DataFrameTransformations":
        ...
        if "transaction_time" in self._df.columns:
            self._df = (
                self._df.withColumn("year", F.year(F.col("transaction_time")))
    ...
    def cast_types(
        ...
        for col_name, type_str in type_mapping.items():
            if col_name in self._df.columns and type_str in type_map:
                self._df = self._df.withColumn(
                    col_name, F.col(col_name).cast(type_map[type_str])
                )
...
if __name__ == "__main__":
    ...
    transformer = DataFrameTransformations(df)
    result = (
        transformer.add_metadata("test_pipeline")
        .add_derived_columns()
