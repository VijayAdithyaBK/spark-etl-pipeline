...
class CustomerSegmentation:
    ...
    def fit(self) -> "CustomerSegmentation":
        ...
        self._customer_df = self.df.groupBy(self.customer_col).agg(
            ...
            F.sum(F.when(F.col("is_night") == 1, 1).otherwise(0)).alias("night_txn_count") if "is_night" in self.df.columns else F.lit(0).alias("night_txn_count"),
            F.sum(F.when(F.col("is_weekend") == 1, 1).otherwise(0)).alias("weekend_txn_count") if "is_weekend" in self.df.columns else F.lit(0).alias("weekend_txn_count"),
        ).withColumn(
            "fraud_rate",
            F.col("fraud_count") / F.col("txn_count")
        ).fillna(0)
        ...
        kmeans = KMeans(k=self.n_clusters, seed=42, featuresCol="features", predictionCol="segment")
    ...
    def _generate_profiles(self):
        ...
        segment_stats = self._customer_df.groupBy("segment").agg(
        ...
        for row in segment_stats:
            ...
            self._profiles.append(SegmentProfile(
