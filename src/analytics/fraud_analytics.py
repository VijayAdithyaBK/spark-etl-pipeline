...
class FraudAnalytics:
    ...
    def _executive_summary(self) -> AnalyticsResult:
        ...
        fraud_amount = self.df.filter(F.col(self.fraud_col) == 1).agg(
            F.sum(self.amount_col)
        ).collect()[0][0] or 0
        
        avg_fraud_amount = self.df.filter(F.col(self.fraud_col) == 1).agg(
            F.avg(self.amount_col)
        ).collect()[0][0] or 0
        
        avg_legit_amount = self.df.filter(F.col(self.fraud_col) == 0).agg(
            F.avg(self.amount_col)
        ).collect()[0][0] or 0
        ...
        data = [{
            ...
            "amount_at_risk_percent": round(fraud_amount / total_amount * 100, 2) if total_amount > 0 else 0,
        ...
        actionable = []
        if fraud_rate > 1:
            actionable.append(f"⚠️ HIGH ALERT: Fraud rate ({fraud_rate:.2f}%) exceeds 1% threshold")
        if avg_fraud_amount > avg_legit_amount * 2:
            actionable.append(f"⚠️ Fraudulent transactions avg ${avg_fraud_amount:.2f} - {avg_fraud_amount/avg_legit_amount:.1f}x higher than legitimate")
    ...
    def _fraud_rate_by_category(self) -> AnalyticsResult:
        ...
        result = (
            ...
            .agg(
                ...
                F.sum(F.when(F.col(self.fraud_col) == 1, 1).otherwise(0)).alias("fraud_count"),
                F.sum(self.amount_col).alias("total_amount"),
                F.sum(F.when(F.col(self.fraud_col) == 1, F.col(self.amount_col)).otherwise(0)).alias("fraud_amount"),
            )
            .withColumn("fraud_rate_percent", F.round(F.col("fraud_count") / F.col("total_transactions") * 100, 4))
    ...
    def _fraud_rate_by_merchant(self) -> AnalyticsResult:
        ...
        actionable = [
            ...
            for m in data[:5] if m["fraud_count"] > 0
        ]
    ...
    def _detect_high_risk_patterns(self) -> AnalyticsResult:
        ...
        amount_stats = self.df.agg(
            F.avg(self.amount_col).alias("avg"),
            F.stddev(self.amount_col).alias("std")
        ).collect()[0]
        ...
        high_value_fraud = self.df.filter(
            ...
            (F.col(self.fraud_col) == 1)
        ).count()
        
        high_value_total = self.df.filter(F.col(self.amount_col) > high_threshold).count()
        high_value_fraud_rate = (high_value_fraud / high_value_total * 100) if high_value_total > 0 else 0
        ...
        patterns.append({
            ...
            "risk_level": "HIGH" if high_value_fraud_rate > 5 else "MEDIUM" if high_value_fraud_rate > 1 else "LOW"
        })
        
        # Pattern 2: Very low value transactions (potential testing)
        low_threshold = 5.0
        low_value_fraud = self.df.filter(
            (F.col(self.amount_col) < low_threshold) & 
            (F.col(self.fraud_col) == 1)
        ).count()
        
        low_value_total = self.df.filter(F.col(self.amount_col) < low_threshold).count()
        low_value_fraud_rate = (low_value_fraud / low_value_total * 100) if low_value_total > 0 else 0
        ...
        patterns.append({
            ...
            "risk_level": "HIGH" if low_value_fraud_rate > 5 else "MEDIUM" if low_value_fraud_rate > 1 else "LOW"
        })
        
        # Pattern 3: Round amounts (potential structuring)
        round_amounts_fraud = self.df.filter(
            (F.col(self.amount_col) % 100 == 0) & 
            (F.col(self.fraud_col) == 1)
        ).count()
        
        round_amounts_total = self.df.filter(F.col(self.amount_col) % 100 == 0).count()
        round_fraud_rate = (round_amounts_fraud / round_amounts_total * 100) if round_amounts_total > 0 else 0
        ...
        patterns.append({
            ...
            "risk_level": "HIGH" if round_fraud_rate > 5 else "MEDIUM" if round_fraud_rate > 1 else "LOW"
        })
        
        actionable = [
            f"📊 {p['pattern']}: {p['fraud_rate_percent']:.2f}% fraud rate - {p['risk_level']} risk"
            for p in patterns if p["risk_level"] in ("HIGH", "MEDIUM")
        ]
    ...
    def _detect_anomalies(self) -> AnalyticsResult:
        ...
        stats = self.df.agg(
            F.avg(self.amount_col).alias("mean"),
            F.stddev(self.amount_col).alias("std")
        ).collect()[0]
        ...
        anomaly_fraud_rate = (fraud_anomalies / total_anomalies * 100) if total_anomalies > 0 else 0
            ...
            "sample_anomalies": [
                {
                    ...
                    "category": row[self.category_col] if self.category_col in row else "N/A"
                }
        ...
        if anomaly_fraud_rate > 10:
            actionable.append(f"🚨 {fraud_anomalies} anomalous transactions are fraudulent ({anomaly_fraud_rate:.1f}%)")
        if total_anomalies > 0:
            actionable.append(f"📌 Set alert threshold at ${mean_amt + 3 * std_amt:,.2f} to flag anomalies")
    ...
    def _analyze_time_trends(self) -> AnalyticsResult:
        ...
        # Add time components
        df_time = self.df.withColumn(
            "hour", F.hour(F.col(self.timestamp_col))
        ).withColumn(
            "day_of_week", F.dayofweek(F.col(self.timestamp_col))
        ).withColumn(
            "month", F.month(F.col(self.timestamp_col))
        )
        ...
        hourly_data = [
            {"hour": row["hour"], "total": row["total"], "fraud_count": row["fraud_count"], "fraud_rate": float(row["fraud_rate"])}
            for row in hourly
        ]
        ...
        daily_data = [
            {"day": day_names[row["day_of_week"]], "total": row["total"], "fraud_count": row["fraud_count"], "fraud_rate": float(row["fraud_rate"])}
            for row in daily
        ]
        ...
        peak_hours = sorted(hourly_data, key=lambda x: x["fraud_rate"], reverse=True)[:3]
        peak_days = sorted(daily_data, key=lambda x: x["fraud_rate"], reverse=True)[:2]
        
        # Build actionable items (avoid nested f-strings)
        peak_hours_str = ", ".join(str(h["hour"]) + ":00" for h in peak_hours) if peak_hours else "N/A"
