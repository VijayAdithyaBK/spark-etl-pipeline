...
class GeographicAnalyzer:
    ...
    def add_geographic_features(self) -> DataFrame:
        ...
        if "state" in df.columns:
            df = df.withColumn(
                "customer_home_state",
                F.first("state").over(customer_window)
            )
            df = df.withColumn(
                "is_out_of_state",
    ...
    def analyze_distance_risk(self) -> list[dict]:
        ...
        ).orderBy(F.col("fraud_rate").desc()).collect()
    ...
    def get_geographic_summary(self) -> dict:
        ...
        if "is_unusual_location" in self.df.columns:
            unusual_stats = self.df.filter(F.col("is_unusual_location") == 1).agg(
                F.count("*").alias("count"),
                F.avg(self.fraud_col).alias("fraud_rate")
            ).collect()[0]
        else:
        ...
        return {
            ...
            "unusual_location_stats": {
                ...
                "fraud_rate_percent": round((unusual_stats["fraud_rate"] or 0) * 100, 2)
            }
        }
