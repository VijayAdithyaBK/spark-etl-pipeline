...
class FeatureEngineer:
    ...
    def add_all_features(self) -> DataFrame:
        ...
        logger.info(
            f"Feature engineering complete. Added {self._feature_count} features."
        )
        return df
    ...
    def add_amount_features(self, df: DataFrame) -> DataFrame:
        ...
        df = df.withColumn("log_amount", F.log1p(F.col(self.amount_col)))

        # Is round amount (potential structuring)
        df = df.withColumn(
            "is_round_amount", F.when(F.col(self.amount_col) % 50 == 0, 1).otherwise(0)
        )
        ...
        df = df.withColumn(
            "customer_avg_amount", F.avg(self.amount_col).over(customer_window)
        )
        df = df.withColumn(
            "customer_std_amount", F.stddev(self.amount_col).over(customer_window)
        )
        df = df.withColumn(
    ...
    def add_velocity_features(self, df: DataFrame) -> DataFrame:
        ...
        df = df.withColumn("customer_txn_count", F.count("*").over(customer_window))

        df = df.withColumn(
            "customer_total_amount", F.sum(self.amount_col).over(customer_window)
        )

        # Recent activity window (last N transactions)
        time_window = Window.partitionBy(self.customer_col).orderBy(self.timestamp_col)

        df = df.withColumn("txn_number", F.row_number().over(time_window))

        # Time since last transaction
        df = df.withColumn(
            "prev_txn_time", F.lag(self.timestamp_col, 1).over(time_window)
        )
        df = df.withColumn(
            "seconds_since_last_txn",
            F.when(
                F.col("prev_txn_time").isNotNull(),
                F.unix_timestamp(F.col(self.timestamp_col))
                - F.unix_timestamp(F.col("prev_txn_time")),
        ...
        df = df.withColumn(
            ...
            F.when(
                ...
                & (F.col("seconds_since_last_txn") < 60),
                1,
            ).otherwise(0),
        )
